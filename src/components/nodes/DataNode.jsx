...
export default function DataNode({ id, data, selected }) {
  ...
  return (
    <div
      className={`bg-white border shadow-sm w-44 transition-all ${selected ? 'ring-2 ring-teal-100' : ''}`}
    ...
    >
      <div className="px-3 py-2.5 flex items-center gap-2 border-b" style={{ backgroundColor: 'rgba(255,255,255,0.6)', borderColor: 'rgba(0,0,0,0.05)' }}>
        ...
        {items.length > 0 && (
          <span
            ...
            style={{ backgroundColor: '#B2DFDB', color: C, borderRadius: 8 }}
          >
            {items.length}
          </span>
        )}
      ...
      <div className="px-3 py-2.5">
        {items.length === 0 ? (
          <div className="text-[11px] text-gray-300 italic">No data</div>
        ) : (
          <div className="flex flex-col gap-0.5">
            {items.map((item) => (
              <div key={item.id} className="relative flex items-center gap-1.5 text-[11px] text-gray-600" style={{ height: 21 }}>
                ...
                {item.format && (
                  <span className="text-gray-400 flex-shrink-0 ml-1">{item.format}</span>
                )}
                <Handle
                  id={`out-${item.id}`}
                  type="source"
